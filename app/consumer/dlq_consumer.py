...
def monitor_dlq():
    ...
    logger.info(f"DLQ Consumer started, monitoring {DLQ_TOPIC}...")
    ...
    while True:
        ...
        if msg is None:
            continue
        if msg.error():
            logger.error("DLQ Consumer error", extra={"error": str(msg.error())})
            continue

        try:
            failed_order = json.loads(msg.value().decode('utf-8'))
            logger.warning("ALARM: Failed order detected in DLQ", extra={
